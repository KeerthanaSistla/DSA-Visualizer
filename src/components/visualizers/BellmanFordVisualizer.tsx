...
import { runBellmanFord, WeightedGraphNode, WeightedGraphEdge, BellmanFordStep } from "@/utils/algorithms/bellmanFord";
...
export const BellmanFordVisualizer = () => {
  ...
  const resetVisualization = () => {
    const bellmanSteps = runBellmanFord(nodes, edges, 0);
    setSteps(bellmanSteps);
    setCurrentStep(0);
    setIsPlaying(false);
  };
  ...
  const currentState = steps[currentStep] || { currentEdge: null, distances: new Map(), iteration: 0 };

  const getNodeColor = (nodeId: number) => {
    if (currentState.currentEdge?.from === nodeId || currentState.currentEdge?.to === nodeId) {
      return "hsl(var(--current))";
    }
      ...
      <Card className="p-4 bg-muted/50">
        <div className="flex flex-wrap items-center gap-4">
          ...
          <div className="text-sm text-muted-foreground">
            Iteration: {currentState.iteration} | Step: {currentStep + 1} / {steps.length}
          </div>
        </div>
      </Card>
        ...
        <div className="relative w-full h-[400px] bg-background/50 rounded-lg border border-border overflow-hidden">
          <svg className="w-full h-full">
            {edges.map((edge, idx) => {
              ...
              const isCurrentEdge = currentState.currentEdge?.from === edge.from && 
                                   currentState.currentEdge?.to === edge.to;
              ...
              return (
                <g key={idx}>
                  <line
                    ...
                    x2={toNode.x}
                    y2={toNode.y}
                    stroke={isCurrentEdge ? "hsl(var(--current))" : "hsl(var(--border))"}
